...
export class PlayCommand extends Command {
	...
	public async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
		...
		if (!validation) {
			return interaction.editReply("Não conheço essa daí não tio")
		...
		if (!channel) {
			return interaction.editReply("tu precisa estar em um canal de voz, tio")
		}
		...
		if (playing.get(guildId) || paused.get(guildId)) {
			...
			return interaction.editReply("Adicionando música a lista")
		}
		...
		return interaction.editReply("Vamos começar :notes:")
	}
}
